...
import { art, avatarStyle, enemyArt, type EnemyKind } from "@/game/data";
  ...
  return (
    <div className={cn("anim-scene relative h-full w-full overflow-hidden", className)}>
      ...
      <div
        ...
        style={{
          background: `linear-gradient(180deg, color-mix(in oklab, var(--sky-deep) ${8 + dim * 40}%, transparent), color-mix(in oklab, var(--sky-deep) ${20 + dim * 50}%, transparent))`,
        }}
      />
      <Clouds />
  ...
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden">
      {[
        ...
        <div
          key={i}
          className="absolute -right-40 opacity-45"
          style={{
            top: c.t,
            transform: `scale(${c.s})`,
            animation: `drift ${c.d}s linear infinite alternate`,
          }}
        >
          <div className="h-16 w-56 rounded-full bg-white/80 blur-[1px]" />
        </div>
      ))}
    </div>
  );
...
export function Hero({ state = "idle", className }: { state?: HeroState; className?: string }) {
  return (
    <div className={cn("relative select-none", className)}>
      {heroBubble[state] && (
        <span className="word-label absolute -top-3 left-1/2 z-10 -translate-x-1/2 text-xs">
          {heroBubble[state]}
        </span>
      )}
      <img
        src={art.hero}
        alt="Your hero, ready to sign"
        loading="lazy"
        width={768}
        height={896}
        className={cn(
      ...
      <span
        aria-hidden
        className="absolute -bottom-2 left-1/2 h-3 w-2/3 -translate-x-1/2 rounded-full bg-ink/25 blur-[3px]"
      />
    </div>
  );
}
  ...
  return (
    ...
    >
      <img
        ...
        width={816}
        height={816}
        className="anim-bob h-20 w-20 shrink-0 object-contain sm:h-24 sm:w-24"
      />
...
export function Avatar({
  ...
  return (
    <span
      aria-hidden
      className={cn("inline-block shrink-0 rounded-full border-[3px] border-ink", className)}
      style={{ ...avatarStyle(index), width: size, height: size, boxShadow: ring }}
    />
  );
}
...
export function EnemySprite({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn("relative grid place-items-center rounded-full", active && "anim-target")}
      ...
      >
        ...
        <img
          src={enemyArt[kind]}
          alt={`${kind} word creature carrying the word ${word}`}
          loading="lazy"
          width={size}
          height={size}
          className={cn(
            "h-full w-full object-contain anim-bob",
            active && "drop-shadow-[0_0_12px_var(--target)]",
          )}
        />
        {fromOpponent && (
