...
export function SentenceQuestDetail({
  ...
  const allLearned = learned.length >= seq.length || (progress?.completions ?? 0) > 0;
          ...
          {mode === "overview" && (
            <div className="mx-auto w-full max-w-5xl space-y-3 pb-6">
              ...
              <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_18rem]">
                <div className="panel space-y-2 p-4">
                  <div className="flex items-center gap-3">
                    <span aria-hidden className="text-6xl anim-bob">
                      {token.emoji}
                    </span>
                    <div>
                      ...
                      <p className="font-display text-[0.6rem] font-bold uppercase tracking-widest text-muted-foreground">
                        Sign reference media placeholder · step {index + 1} of {seq.length}
                      </p>
                    </div>
                  </div>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                ...
                >
                  {allLearned ? "Practise Full Sentence" : "Learn every sign to unlock"}
                </GameButton>
...
function FullSentencePractice({
  ...
  const finish = useCallback(
    (opts: { completed: boolean; orderPct: number; flowScore: number; message: string }) => {
      ...
      setAttempts((a) => a + 1);
      setStreak((s) => (opts.completed ? s + 1 : 0));
      ...
      <div className="grid gap-3 md:grid-cols-[16rem_minmax(0,1fr)]">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <HudChip label="Sentence streak" value={streak} tone="success" />
            <HudChip
              label="Position"
    ...
    <div className="mx-auto w-full max-w-4xl space-y-3 pb-6">
      ...
      <div className="flex flex-wrap justify-center gap-2">
        ...
        <GameButton
          tone="magic"
          onClick={() =>
            setHint(`Clue: sign ${tokenById(seq[0]).name.toUpperCase()} first, then follow the meaning.`)
          }
        >
