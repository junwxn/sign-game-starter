...
import { ArrowLeft, ArrowRight, Heart, Search, Star } from "lucide-react";
import { CoachBubble, GameButton, IconButton, Meter, Scene, Stars } from "@/components/game/kit";
  ...
  return (
    <Scene dim={0.3}>
      <div className="flex h-full flex-col p-3 sm:p-4">
        ...
        <ul className="mt-3 grid flex-1 grid-cols-2 content-start gap-3 overflow-y-auto pb-4 sm:grid-cols-3 lg:grid-cols-4">
          {list.map((s) => {
            const m = mastery[s.id];
            const weak = focusSigns?.includes(s.id);
            const locked = !m && s.category === "People";
            return (
              <li key={s.id}>
                <button
                  type="button"
                  ...
                  className={cn(
                    "btn-game w-full flex-col !items-center gap-1 !px-3 !py-4",
      ...
      <div className="h-full overflow-y-auto p-3 sm:p-4">
        <div className="mx-auto w-full max-w-3xl space-y-3 pb-6">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="panel grid place-items-center gap-1 p-6">
              <span aria-hidden className="text-7xl anim-bob">
                {sign.emoji}
              </span>
              <p className="font-display text-xs font-bold uppercase tracking-widest text-muted-foreground">
                Sign reference media placeholder
              </p>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2 text-xs">
                    ...
                    <span className="hud-chip">
                      <Heart className="h-3.5 w-3.5 text-danger" aria-hidden /> Streak {streak}
                    </span>
                  </div>
