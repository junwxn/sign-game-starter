...
export function Battle({
  ...
  useTicker(
    () => {
      setEnemies((list) =>
        list.filter((e) => e.status !== "defeated").length >= MAX_ENEMIES
          ? list
  ...
  useTicker(
    () => {
      ...
      if (roll < skill) {
        ...
        if (hard) setOppMeter((m) => Math.min(100, m + 14 + Math.random() * 10));
      } else {
        setOppCombo(0);
        if (hard) setOppMeter((m) => Math.max(0, m - 8));
      }
    },
  ...
  useEffect(() => {
    ...
    setWarning(count > 1 ? `${count} ENEMIES INCOMING!` : "INCOMING WORD!");
    setAttackFx("in");
    ...
    setTimeout(() => {
      ...
      if (kind === "correct") {
        ...
        if (target.fromOpponent) {
          ...
          streakDefence.current += 1;
          if (hard) setMeter((m) => Math.min(100, m + 8));
          if (streakDefence.current === 2) addFloat("NICE DEFENCE!", "magic", 50, 34);
          if (streakDefence.current === 3) {
